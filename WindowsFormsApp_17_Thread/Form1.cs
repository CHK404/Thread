 ...
 *ㄴ 백그라운드에서 별도 작업 실행 가능
 * 
 *쓰는 이유
 *1) 긴 작업(다운로드, 대규모 계산 등)을 실행할 때 UI가 멈추지 않도록
 *2) 여러 작업을 동시에 실행하고 싶을 때(병렬 처리)
 *
 *대표 메서드
 *-Start(): 스레드 실행
 *-Sleep(ms): 지정 시간(ms) 동안 현재 스레드 일시 정지
 *-Abort(): 스레드 강제 종료(권장 x)
 ...
 *
 *UI 스레드(=메인 스레드)
 *-프로그램 실행 시작과 동시에 생성(폼, 버튼, 텍스트박스 등 UI 담당)
 */
        ...
        private void UpdateData1()
        {
            lock (lockObject)
            {
                for (int i = 0; i < 10; i++)
                {
                    ...
                    //bool 타입 속성
                    //true면 지금 코드가 UI 스레드가 아닌 다른 스레드에서 실행중이라는 뜻
                    {
                        //UI 업데이트는 메인(UI)스레드에서 실행되도록 위임
                        textBox1.Invoke(new MethodInvoker(() =>
                        //Invoke()는 UI 스레드에게 이 메서드 대신 실행 요청하는 메서드
                        //MethodInvoker
                        //ㄴ 매개변수 없고 반환값도 없는 메서드를 UI 스레드에서 실행하도록 함
                        {
